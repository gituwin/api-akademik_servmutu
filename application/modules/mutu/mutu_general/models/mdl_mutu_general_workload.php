...
class Mdl_mutu_general_workload extends CI_Model
{
    public function __construct()
    {
        parent::__construct();
        $this->mutu = $this->load->database('workload');
    }

    public function get_data_v4($kd_prodi, $kd_ta, $kd_smt)
    {
        $beban_krng = 'Beban_Kurang';
        $beban_Sesuai = 'Beban_Sesuai';
        $beban_lebih = 'Beban_lebih';

        // Perbaiki penulisan $kk dan tambahkan spasi setelah WHERE
        $kk = "kd_prodi = '$kd_prodi' AND ";
        if ($kd_prodi == 'eryftu') {
            $kk = '';
        }

        $query = "SELECT kd_kelas, nm_mk, kelas_pararel, COUNT(nim) AS total_mhs, kd_mk,
        COUNT(CASE WHEN out_put_text::text = 'Beban Kurang' THEN 1 END) AS $beban_krng,
        COUNT(CASE WHEN out_put_text::text = 'Beban Sesuai' THEN 1 END) AS $beban_Sesuai,
        COUNT(CASE WHEN out_put_text::text = 'Beban lebih' THEN 1 END) AS $beban_lebih,
        id_soal
        FROM workload.m_jawaban
        WHERE $kk kd_ta = $kd_ta AND kd_smt = $kd_smt AND out_put_text IS NOT NULL
        GROUP BY kd_kelas, nm_mk, kelas_pararel, id_soal, kd_mk
        ORDER BY kd_kelas, nm_mk";
    ...
    public function get_data_v5($kd_prodi, $kd_ta, $kd_smt)
    {
        $beban_krng = 'Beban_Kurang';
        $beban_Sesuai = 'Beban_Sesuai';
        $beban_lebih = 'Beban_lebih';
    ...
    public function get_data($par)
    {
        ...
        if (!empty($par['GROUP_BY'])) {
            $this->mutu->group_by($par['GROUP_BY']);
        }
