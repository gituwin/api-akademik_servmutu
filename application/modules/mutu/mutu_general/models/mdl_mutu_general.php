...
class Mdl_mutu_general extends CI_Model
{
    ...
    public function insert_data($par)
    {
        ...
        // Menangani input tanggal dengan TO_DATE
        if (isset($data['tanggal'])) {
            // Ganti dengan format yang sesuai untuk PostgreSQL

            foreach ($data['tanggal'] as $nama_kolom => $nilai) {
                // Gunakan TO_DATE untuk nilai yang memerlukan konversi ke tanggal
                $this->mutu->set($nama_kolom, "TO_DATE('{$nilai}', 'DD-MON-YY')", false);
            }
            unset($data['tanggal']); // Jangan masukkan tanggal ke array data, karena sudah di-handle oleh `set()`
        }
    ...
    public function count_data($par = '')
    {
        ...
        if (!empty($par['WHERE_BETWEEN'])) {
            foreach ($par['WHERE_BETWEEN'] as $key => $val) {
                if (isset($val['start']) && isset($val['end'])) {
                    $this->mutu->where($key . ' >=', $val['start']);
                    $this->mutu->where($key . ' <=', $val['end']);
                }
            }
        }
